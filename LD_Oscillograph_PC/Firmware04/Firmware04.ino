...
// Прошивка Ардуино ProMicro v.0.4a
// Страница проекта:
...
// Rev.2 11.11.2022 (0.3a) Оптимизация протокола передачи измерений
// Rev.3 17.01.2023 (0.4a) Исключен баг с передачей отрицательных значений в ПК
...
void SerialCommand(){ // Обработка дистанционного управления
  ...
  if (Serial.available()||Serial1.available()){
    ...
    if (CMD == 'V') 
      if (useBT) 
            Serial1.println(VER);  //Текущая версия
       else
            Serial.println(VER);  //Текущая версия
...
void SendValue24(byte CH) { // Процедура измерения и отправки числа в порт
  ...
  switch (CH){
    case 1: // CH1
    ads1220.select_mux_channels(MUX_SE_CH0); // Выбор 1 канала
    adc_data=ads1220.Read_WaitForData(); // Чтение измерения
    break;
    case 2: // CH2
    ads1220.select_mux_channels(MUX_SE_CH1); // Выбор 2 канала
    adc_data=ads1220.Read_WaitForData(); // Чтение измерения
    break;
    case 3: // CH3
    ads1220.select_mux_channels(MUX_SE_CH2); // Выбор 3 канала
    adc_data=ads1220.Read_WaitForData(); // Чтение измерения
    break;
    case 4: // CH4
    ads1220.select_mux_channels(MUX_SE_CH3); // Выбор 4 канала
    adc_data=ads1220.Read_WaitForData(); // Чтение измерения
    break;
  }
  if (useBT){
...
void SendValue8(byte CH) { // Процедура измерения и отправки оного числа в порт
  ...
  if (useBT){
    //Serial1.print(CH+4);
    Serial1.print(",");
    Serial1.print(D_data, HEX);
  } else {
    //Serial.print(CH+4);
    Serial.print(",");
    Serial.print(D_data, HEX);
  }
}
